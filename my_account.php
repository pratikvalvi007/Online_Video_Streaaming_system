		...
		<form method="get" action="search.php" enctype="multipart/form-date">
			...
			<div id="shopping_cart"> 
				...
				<div id="products_box">
					...
					$get_wl="select * from watchlist NATURAL JOIN series NATURAL JOIN auser where username='$email'";
					$run_wl= mysqli_query($con,$get_wl);
					
					while($row_series=mysqli_fetch_array($run_wl)){
					$series_title = $row_series['series_title'];
					$username  = $row_series['username'];
					$uemail = $row_series['email'];
					
					
					
					echo "
					<h3>User Name : $username </h3><br>
					<h3>Email : $uemail</h3>
					<a href='changepass.php?username=$username'><button style='float:center'>Change Password</button></a>
